...
#
# 元 DGX2 8GPU bs=128 → global=1024。DGX1 16GPU で per-rank bs=64 にして
# global=1024 を保つ。
#
# Reproduce (3 通り):
#   (A) ClearML queue=dgx1 経由 (推奨、agent が動いてれば):
...
#
#   (B) DGX1 上で直接 docker run (agent が止まってる時の fallback):
#       ssh dgx1 'docker run -d --gpus all --shm-size=64g \
...
#         --name calib_repro e2e-calib-train:np2 \
#         bash -c "accelerate launch --num_processes=16 --mixed_precision=fp16 \
#           scripts/training/train_ps_v3_ddp.py \
#           --name km_wv_wm_n4_img128_grid16_repro_dgx1_16gpu \
#           --cache /home/hfunaya/cache_v4/kamikado_v3_tiled,/home/hfunaya/cache_v4/woven_v3_tile,/home/hfunaya/cache_v4/waymo_v3_tiled_i \
#           --epochs 200 --batch-size 64 --img-size 128 --n-layers 4 --convnext \
#           --grid-n 16 --deform-mode sl --rot-deg 1.5 --t-m 0.20 --oversample 4 \
#           --workers 8 --min-crop-px 256 --max-crop-px 512 \
#           --lr 0.001 --lr-min 1e-06 \
#           --ba-eval-start-ep 1 --ba-eval-every 10 --ba-eval-n-seeds 4 \
#           --ba-eval-n-inst 200 2>&1 | tee /home/hfunaya/calib_repro.log"'
#       監視: ssh dgx1 'tail -f /home/hfunaya/calib_repro.log'
#       停止: ssh dgx1 'docker stop calib_repro && docker rm calib_repro'
#
#   (C) 単一 GPU の sanity smoke (新規 host):
