"""Train CalibNetDepth on sim3d data (max_offset=8px)."""
import sys, pathlib; sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
import math, time, torch, torch.nn as nn
from datasets.sim3d import build_loaders_sim3d_cached
...
def epoch_loop(model, loader, optimizer, scaler, train):
    ...
    for imgs, true_uvd, dist_uvd in loader:
        ...
        gt       = true_uvd[..., :2] - dist_uvd[..., :2]   # UV offset only
...
def main():
    print(f"sim3d training  max_offset={MAX_OFFSET}px  bg_ratio={BG_RATIO}", flush=True)
    train_loader, val_loader = build_loaders_sim3d_cached(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"[{epoch:3d}/{EPOCHS}]  "
              ...
              f"tot={(time.time()-t0)/60:.1f}min", flush=True)
        if va_nll < best_val:
            best_val = va_nll
            raw = model._orig_mod if hasattr(model, "_orig_mod") else model
            torch.save(raw.state_dict(), CKPT)
            print(f"  ↳ saved (val_nll={best_val:.4f})", flush=True)

    print(f"\nBest val NLL: {best_val:.4f}  |  time: {(time.time()-t0)/60:.1f}min", flush=True)
