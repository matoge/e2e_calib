"""Train CalibNetDepth on PandaSet real data."""
...
def main():
    ...
    cache = '/tmp/pandaset_full_cache.pt'
    kw = dict(num_workers=4, pin_memory=True, persistent_workers=True,
              collate_fn=collate_pandaset)
    train_ds = PandaSetCalibDataset(cache, split='train')
    val_ds   = Subset(train_ds, range(0, len(train_ds), 10))
    ...
    model = CalibNetDepth(img_size=c["img_size"], in_channels=c["in_channels"],
                          ...
                          use_convnext=c.get("use_convnext", False)).to(DEVICE)
    ...
    for vi in range(n_vis):
        ...
        is_obj  = dist_uvd[:, 3].numpy() > 0.5   # bool (N,)
        ...
        for mask, col_gt, col_d, col_p, lbl in [
            ...
            ax.scatter(true_uv[mask,0], true_uv[mask,1],
                       c=col_gt, s=20, marker='x', linewidths=1.2, label=f'GT {lbl}', zorder=3)
            ax.scatter(dist_uv[mask,0], dist_uv[mask,1],
                       c=col_d,  s=10, alpha=0.5, label=f'dist {lbl}', zorder=2)
            ax.scatter(pred_uv[mask,0], pred_uv[mask,1],
                       c=col_p,  s=20, marker='+', linewidths=1.4, label=f'pred {lbl}', zorder=4)
            for dx,dy,px,py in zip(dist_uv[mask,0], dist_uv[mask,1],
                                   pred_uv[mask,0], pred_uv[mask,1]):
                ax.annotate("", xy=(px,py), xytext=(dx,dy),
                            arrowprops=dict(arrowstyle="->", color=col_p, lw=0.5, alpha=0.5))

        ax.set_title(f"obj:{err_b_obj:.2f}→{err_a_obj:.2f}  bg:{err_b_bg:.2f}→{err_a_bg:.2f}px",
                     fontsize=6)
