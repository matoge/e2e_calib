"""Overfit smoke test for multicrop PandaSet dataset.

Verifies that the variable sub-crop augmentation doesn't kill learning —
NLL should still descend cleanly on a small dense subset.
"""
...
from datasets.pandaset import PandaSetCalibDataset, collate_pandaset
...
CACHE = "/mnt/nvme6t/e2e_calib_cache/pandaset_mc_cache.pt"

CFG = dict(
    name          = "ps_mc_overfit500",
    n_layers      = 4,
    img_size      = 64,
    in_channels   = 3,
    use_convnext  = True,
...
def main(cfg=None):
    ...
    tr_full = PandaSetCalibDataset(CACHE, split='train')
    va_full = PandaSetCalibDataset(CACHE, split='val')
    ...
    if c.get("min_pts"):
        ...
        idxs = [i for i in idxs if _dense(i)]
        log(f"min_pts={mp} filter: kept {len(idxs)} instances")

    if c.get("subset_size"):
        idxs = idxs[:int(c["subset_size"])]

    train_idxs = idxs
    val_idxs   = idxs if c.get("overfit") else idxs[:max(1, len(idxs)//10)]
    log(f"overfit={c.get('overfit', False)}  train={len(train_idxs)}  val={len(val_idxs)}")
    ...
    axes[0].plot(history['ep'], history['tr_nll'], label='train')
    axes[0].plot(history['ep'], history['va_nll'], label='val')
    axes[0].set_title('NLL'); axes[0].set_xlabel('epoch'); axes[0].legend(); axes[0].grid(alpha=0.3)
    axes[1].plot(history['ep'], history['tr_mse'], label='train')
    axes[1].plot(history['ep'], history['va_mse'], label='val')
