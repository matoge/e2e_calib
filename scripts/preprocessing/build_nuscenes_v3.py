...
def _ann_to_cuboid(ann) -> dict:
    """nuScenes annotation → {pos, dims, yaw, label} in world frame.
    Matches PandaSet/Waymo: 'pos'/'dims' (not 'center'/'size') so that
    _is_obj_per_point and pack_cubs both work."""
    ...
    dims = np.array([sz[1], sz[0], sz[2]], dtype=np.float32)
    q = ann['rotation']  # [w, x, y, z]
    R = _quat_mat(q)
    yaw = float(np.arctan2(R[1, 0], R[0, 0]))
    return dict(pos=pos, dims=dims, yaw=yaw, label='')
