...
import sys, pathlib, argparse; sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
...
ap.add_argument('--n-segs',  type=int, default=1, help='how many segments to render')
ap.add_argument('--frames',  default='0,40,100', help='comma-separated frame indices per seg')
ap.add_argument('--out',     default='vis_waymo_full_proj.png')
args = ap.parse_args()

FRAMES = [int(x) for x in args.frames.split(',')]
N_SEGS = args.n_segs

all_segs = sorted(f.stem for f in (WAYMO_DIR/'lidar').glob('*.parquet'))
segs_use = all_segs[:N_SEGS]
print(f'segments: {len(segs_use)}  frames/seg: {FRAMES}')
...
nrows = len(segs_use) * len(FRAMES)
fig, axes = plt.subplots(nrows, 1, figsize=(20, 11*nrows), dpi=70)
...
for seg in segs_use:
    ...
    for fi in FRAMES:
        ...
        ax.set_title(f'{seg[:40]}..  frame={fi}  lidar={int(vis.sum())}/{len(pts_veh)}  {label}', fontsize=11)

if sc is not None:
    cbar = fig.colorbar(sc, ax=axes, shrink=0.5, label='depth (m)')
plt.savefig(args.out, dpi=70, bbox_inches='tight')
