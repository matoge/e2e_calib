...
For each LiDAR point $i$, the model returns
...
$\mathbf{r}_i$ is the per-point correction we want to apply to the
observed uv to land on GT. $\Sigma_i$ is the model's uncertainty on
that correction. We want to explain it all with a single 6-DoF rig
offset $\boldsymbol{\delta} = (\omega_x,\omega_y,\omega_z,t_x,t_y,t_z)$.

$\Sigma_i$ is not a scalar — it's a **2-D covariance** that encodes
...
it can. BA uses $\Sigma_i$ as a Mahalanobis weight, so only the
information-rich directions of each point actually enter the solve.

### Linearisation

When the cam-frame point $\mathbf{X}_i = (X_i, Y_i, Z_i)$ moves under
$\boldsymbol{\delta}$, the image-plane displacement to first order is
...
It's just **"if $u$ at point $i$ moves a tiny bit, how much does each
of the six camera parameters explain that movement under
linearisation?"**. A 1° yaw moves a centre point by $f_x \approx 33$
...
- The per-pt sensitivity matrix $J_i \in \mathbb{R}^{2 \times 6}$ is
  only **rank 2** (one point cannot pin all 6 DoF).
- Stack thousands of points spread across the frame and
  $\sum_i J_i^\top \Sigma_i^{-1} J_i$ becomes full rank (regular 6×6).
- Invert and out pops $\boldsymbol{\delta} = H^{-1}\mathbf{b}$.
...
For KB fisheye, $u = f_x \cdot \theta_d \cdot X/r + c_x$ with
$\theta_d = \theta(1+k_1\theta^2+\dots+k_4\theta^8)$. Same chain rule
runs through and we get an analytic $J_i$ in
`scripts/ba/ba_kb_jac.py:KB_DOF_JAC`.

### Mahalanobis-weighted normal equation

We minimise the sum of whitened per-pt residuals
$\Sigma_i^{-1/2}(\mathbf{r}_i - J_i\boldsymbol{\delta})$:
...
$\boldsymbol{\delta}^* = H^{-1}\mathbf{b}$ in one step. The covariance
$\mathrm{Cov}(\boldsymbol{\delta}^*) = H^{-1}$ falls out for free.

### Huber IRLS for outlier suppression

Compute the per-pt Mahalanobis distance
$d_i^2 = (\mathbf{r}_i - J_i\boldsymbol{\delta})^\top \Sigma_i^{-1}(\mathbf{r}_i - J_i\boldsymbol{\delta})$,
scale $\Sigma_i^{-1}$ by $w_i = \min(1, k/d_i)$, re-solve. The
...
- **KB closed-form (`solve_dofs_kb`)**: new in
  ...
  KB projection $\theta_d = \theta(1+k_1\theta^2+\dots+k_4\theta^8)$
