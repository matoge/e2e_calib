...
- 静的物体が前景物 (太いポール、車など) の **裏に隠れる場合**、本手法でも
  ...
  uv は幾何で正しく計算でき、Δuv ≈ 0 で出力できる
- 「画像で見えるか」は出力には直接関係せず、「pose 仮説下の射影位置と
  実際のずれ」だけを学習している

つまり遮蔽点も **「正しい位置 + 低 σ」** で残せる = 落ちない。

ただし「情報量がフル」とは違う:

- 遮蔽点では B 画像の該当 uv に **本来の物体の image feature がない**
  (前景物が映ってるだけ)、その点については image 側のエビデンスはほぼ無い
- モデルが頼るのは **pose 仮説 + LiDAR (= geometry)** のみ。普通の点なら
  image + LiDAR の両 modality で確信を取るところを、片肺になる
- それでも matcher の「対応なし → 完全削除」よりは情報量がある: geometry
  だけでも uv 推定 + σ は出るので、down-stream で重み付けて使える
