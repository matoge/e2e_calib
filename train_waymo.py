"""Train CalibNetDepth on Waymo Open Dataset v2."""
import math, time, torch, torch.nn as nn
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
from pathlib import Path
from datetime import datetime
from dataset_waymo import WaymoCalibDataset, collate_waymo, build_cache
...
CACHE = '/tmp/waymo_train_cache.pt'
...
def main():
    ...
    if not Path(CACHE).exists():
        log("Building Waymo cache...")
        build_cache(CACHE)

    kw = dict(num_workers=4, pin_memory=True, persistent_workers=True,
              collate_fn=collate_waymo)
    train_ds = WaymoCalibDataset(CACHE, split='train')
    val_ds   = WaymoCalibDataset(CACHE, split='val')
    ...
    for vi in range(n_vis):
        ...
        ax.set_title(f"obj:{err_b_obj:.2f}→{err_a_obj:.2f}  bg:{err_b_bg:.2f}→{err_a_bg:.2f}px", fontsize=6)
