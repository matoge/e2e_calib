...
def main():
    print("Multi-object training (2 independent shifts)")
    train_loader, val_loader = build_loaders_multi(
        train_size=8000, val_size=800, batch_size=BATCH_SIZE, num_workers=4)

    model = torch.compile(CalibNet().to(DEVICE), mode="max-autotune")
    ...
    print(f"\nBest val: {best_val:.4f}  |  time: {(time.time()-t0)/60:.1f}min")
